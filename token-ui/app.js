...
const app = new web3.Web3(web3.givenProvider || "http://localhost:8545");
...
const simpleTokenContract = new app.eth.Contract(contractABI, contractAddress);
...
document.getElementById('transferButton').addEventListener('click', async () => {
    ...
    try {
        const accounts = await app.eth.getAccounts();
        const sender = accounts[0];
        
        await simpleTokenContract.methods.transfer(recipient, app.utils.toWei(amount, 'ether')).send({ from: sender });

        alert('Transfer successful!');
    } catch (error) {
        console.error('Error while transferring:', error);
        alert('Error while transferring. See console for details.');
    }
});
